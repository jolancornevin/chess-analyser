...
function MoveUX({ move, onClick, currentMoveID, orientation }: MoveProps): JSX.Element {    
    const noteMove = useCallback((move: Move): JSX.Element => {
        ...
        if (move.scoreDiff < 60) {
            ...
            score.push(<>{move.scoreBefore > move.scoreAfter && <img height={15} src="img/mistake.png" alt="mistake"/>}.</>);
        } else if (move.scoreDiff < 150) {
            score.push( <>{move.scoreBefore > move.scoreAfter && <img height={15} src="img/misstake.png" alt="misstake"/>}.</>);
        } else {
            if (move.wasOnlyMove && !move.playedOnlyMove) {
                ...
                score.push(<>{move.scoreBefore > move.scoreAfter && <img height={15} src="img/blunter.png" alt="blunter"/>}.</>);
            }
        }
